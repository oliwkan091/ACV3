...
def remove_link(metaFileNames):
	links = Dict.loadDataFromFile(metaFileNames)
	if Dict.checkIfDicionEelementExists(links, "title"):
		choice = make_choice('Wybierz który link chcesz usunąć', links["title"])
...
def show_exceptions(metaFileNames):
	...
	if Dict.checkIfDicionEelementExists(links,"title"):
		choice = make_choice('Z jakiego pliku wyświetlić wyjątki', links["title"])
...
def add_exception(metaFileNames):
	...
	if Dict.checkIfDicionEelementExists(links, "title"):
		choice = make_choice("Wybierz do jakiego pliku chcesz dodać wyjątek",links["title"])
		typeChoice = make_choice(Dict.makeNameFromLink(links["title"][choice-1], 'txt') + '   Podaj typ wyjątku', Dict.exceptionTypes)
		excInp = input('Podaj wyjątek: ')
		excFile = Dict.loadDataFromFile(Dict.metaFileNames['database'] + '\\' + Dict.makeNameFromLink(links["title"][choice-1],'txt'))
		if not Dict.checkIfDicionEelementExists(excFile,"block") or not Dict.checkIfDicionEelementExists(excFile,"key"):
			excFile["block"] = []
			excFile["key"] = []

		excFile[Dict.exceptionTypes[typeChoice-1]].append(excInp)

		Dict.saveDataToFile(Dict.metaFileNames['database'] + '\\' + Dict.makeNameFromLink(links["title"][choice-1],'txt'),excFile)
...
def remove_exception(metaFileNames):
	...
	if Dict.checkIfDicionEelementExists(links, "title"):
		...
		choice = make_choice("Wybierz z jakiej bazy chcesz usunąć wyjątek",links["title"])

		exceptions = Dict.loadDataFromFile(Dict.metaFileNames['database'] + '\\' + Dict.makeNameFromLink(links["title"][choice - 1], "txt"))
		print(exceptions)
		if exceptions["block"] != [] and exceptions["key"] != []:
			typeChoice = make_choice("Który typ chcesz usunąć", Dict.exceptionTypes)
			toDel = make_choice("Który wyjątek chcesz usunąć", exceptions[Dict.exceptionTypes[typeChoice-1]])

			exceptions[Dict.exceptionTypes[typeChoice-1]].pop(toDel-1)
		else:
			if exceptions["block"] != []:
				toDel = make_choice("Który wyjątek chcesz usunąć", exceptions[Dict.exceptionTypes[0]])
				exceptions[Dict.exceptionTypes[0]].pop(toDel-1)
			else:
				toDel = make_choice("Który wyjątek chcesz usunąć", exceptions[Dict.exceptionTypes[1]])
		...
		if exceptions["block"] == [] and exceptions["key"] == []:
			...
			Dict.saveDataToFile(Dict.metaFileNames['database'] + '\\' + Dict.makeNameFromLink(links["title"][choice-1], "txt"),exceptions)
	else:
	...
	if Dict.isFile(Dict.metaFileNames['pages']):
		...
		if len(links) != 0:
			...
			while i < linksLen:
				...
				i += 1
			choice = make_choice('Wybierz link którego strona pochodna ma zostać sprawdzona', links)
...
if __name__ == '__main__' :
	...
	choice = 0
	while True:
		choice = make_choice('Wybierz co chcesz zrobić', Dict.mainMenu)
